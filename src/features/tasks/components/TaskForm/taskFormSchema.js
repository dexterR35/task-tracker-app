...
export const TASK_FORM_CONFIG = {
  ...
  TIME_INPUT: {
    MIN_HOURS: 0.5,
    MAX_HOURS: 999, // Removed 24hr limit
    STEP_SIZE: 0.5,
  },
  ...
  // Form reset values
  DEFAULT_VALUES: {
    TIME_IN_HOURS: 0.5, // Changed from 0 to 0.5
    AI_TIME_SPENT: 0.5,
...
export const validateTimeInput = (timeInHours) => {
  if (timeInHours === null || timeInHours === undefined || timeInHours === '') {
    return false;
  }
...
export const formatTimeInput = (timeInHours) => {
  ...
  return Math.max(
    TASK_FORM_CONFIG.TIME_INPUT.MIN_HOURS,
...
export const validateArrayLength = (
  array,
  maxLength = TASK_FORM_CONFIG.VALIDATION.MAX_ARRAY_LENGTH
) => {
  return Array.isArray(array) && array.length <= maxLength;
};

export const sanitizeStringInput = (
  input,
  maxLength = TASK_VALIDATION_CONFIG.NAME.MAX_LENGTH
) => {
...
export const taskFormSchema = Yup.object().shape({
  jiraLink: Yup.string()
    ...
    .test(
      "jira-link",
      "Must be a valid Atlassian Jira URL (e.g., https://company.atlassian.net/browse/TASK-123)",
      (value) => {
  ...
  taskNumber: Yup.string()
    .matches(
      VALIDATION_PATTERNS.TASK_NUMBER,
      "Task number must be in format TASK-123 (e.g., PROJ-456)"
    )
    .max(
      TASK_VALIDATION_CONFIG.TASK_NUMBER.MAX_LENGTH,
      `Maximum ${TASK_VALIDATION_CONFIG.TASK_NUMBER.MAX_LENGTH} characters`
    )
    .test("task-number-format", "Invalid task number format", (value) =>
      validateTaskNumber(value)
    )
  ...
  markets: Yup.array()
    ...
    .test(
      "array-length",
      `Maximum ${TASK_FORM_CONFIG.VALIDATION.MAX_ARRAY_LENGTH} markets allowed`,
      (value) => validateArrayLength(value)
    )
    .required("Required"),

  products: Yup.string().required("Please select a product"),

  departments: Yup.string().required("Required"),

  timeInHours: Yup.number()
    .typeError("Time must be a valid number")
    .min(
      TASK_FORM_CONFIG.TIME_INPUT.MIN_HOURS,
      `Time must be at least ${TASK_FORM_CONFIG.TIME_INPUT.MIN_HOURS} hours`
    )
    .max(
      TASK_FORM_CONFIG.TIME_INPUT.MAX_HOURS,
      `Time cannot exceed ${TASK_FORM_CONFIG.TIME_INPUT.MAX_HOURS} hours`
    )
    .test("time-validation", `Time must be at least ${TASK_FORM_CONFIG.TIME_INPUT.MIN_HOURS} hours`, (value) => {
      if (value === null || value === undefined) return false;
      const num = parseFloat(value);
      if (isNaN(num)) return false;
      return num >= TASK_FORM_CONFIG.TIME_INPUT.MIN_HOURS;
    })
  ...
  aiModels: Yup.array().when("usedAI", {
    is: true,
    then: (schema) => schema.min(1, "AI models are required when AI is used"),
    otherwise: (schema) => schema,
  }),

  aiTime: Yup.number().when("usedAI", {
    is: true,
    then: (schema) => schema
      .min(TASK_FORM_CONFIG.TIME_INPUT.MIN_HOURS, `AI time must be at least ${TASK_FORM_CONFIG.TIME_INPUT.MIN_HOURS} hours`)
      .max(TASK_FORM_CONFIG.TIME_INPUT.MAX_HOURS, `AI time cannot exceed ${TASK_FORM_CONFIG.TIME_INPUT.MAX_HOURS} hours`)
      .test("ai-time-validation", `AI time must be at least ${TASK_FORM_CONFIG.TIME_INPUT.MIN_HOURS} hours`, (value) =>
        validateTimeInput(value)
      )
      .required("AI time is required when AI is used"),
    otherwise: (schema) => schema,
  }),

  reporters: Yup.string().required("Please select a reporter"),
});
...
export const getTaskFormInitialValues = (
  ...
  const defaults = {
    ...
    usedAI: false,
    aiModels: [],
    aiTime: TASK_FORM_CONFIG.DEFAULT_VALUES.AI_TIME_SPENT,
    reporters: "",
  };
  ...
  return {
    jiraLink: sanitizeStringInput(
      initialValues.jiraLink || "",
      TASK_VALIDATION_CONFIG.URL.MAX_LENGTH
    ),
    taskNumber: formatTaskNumber(initialValues.taskNumber || ""),
    markets: Array.isArray(initialValues.markets)
    ...
    timeInHours: formatTimeInput(
      initialValues.timeInHours || defaults.timeInHours
    ),
    // Auto-check hasDeliverables if deliverables array has data
    hasDeliverables: Boolean(
      initialValues.hasDeliverables ||
        (Array.isArray(initialValues.deliverables) &&
          initialValues.deliverables.length > 0)
    ),
    deliverables: Array.isArray(initialValues.deliverables)
      ? initialValues.deliverables
      : defaults.deliverables,
    // Auto-check usedAI if aiModels array has data
    usedAI: Boolean(
      initialValues.usedAI ||
        (Array.isArray(initialValues.aiModels) &&
          initialValues.aiModels.length > 0)
    ),
    aiModels: Array.isArray(initialValues.aiModels)
      ? initialValues.aiModels
      : defaults.aiModels,
    aiTime:
      typeof initialValues.aiTime === "number"
        ? initialValues.aiTime
        : defaults.aiTime,
