...
const setupAuthPersistence = async (maxRetries = 3) => {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      logger.warn(
        `Persistence setup attempt ${attempt} failed, retrying in ${delay}ms...`
      );
...
const initializeAuthPersistence = async () => {
  try {
    ...
    if (typeof window !== "undefined" && typeof document !== "undefined") {
      ...
      logger.log(
        "Skipping auth persistence setup - not in browser environment"
      );
    }
  } catch (error) {
    ...
    if (import.meta.env.MODE === "development") {
      logger.warn(
        "Firebase auth persistence is not available. Some features may not work correctly."
      );
    }
  }
};
