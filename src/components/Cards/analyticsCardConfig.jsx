...
export const calculateMarketsByUsersData = (
  tasks,
  users,
  options = CALCULATION_OPTIONS.FULL_MARKETS_BY_USERS
) => {
  if (!tasks || tasks.length === 0) {
    return {
      tableData: [
        {
          user: "No data availables",
          totalTasks: 0,
          totalHours: "0h",
          noData: true,
        },
      ...
      tableColumns: [
        ...
        {
          key: "totalTasks",
          header: "Total Tasksssss",
          align: "center",
          highlight: true,
        },
        {
          key: "totalHours",
          header: "Total Hourssss",
          align: "center",
          highlight: true,
        },
      ],
...
const calculateUsersBiaxialData = (tasks, users) => {
  ...
  tasks.forEach((task) => {
    ...
    const createdBy =
      task.data_task?.createdBy ||
      task.createdBy ||
      task.data_task?.createdByName ||
      task.createdByName;
    const assignedTo =
      task.data_task?.assignedTo ||
      task.assignedTo ||
      task.data_task?.assignedToName ||
      task.assignedToName;
    const reporter =
      task.data_task?.reporter ||
      task.reporter ||
      task.data_task?.reporterName ||
      task.reporterName;
...
export const getMarketsByUsersCardProps = (
  tasks,
  users,
  isLoading = false,
  options = CALCULATION_OPTIONS.FULL_MARKETS_BY_USERS
) => {
  // The logic for early returning empty data when tasks or users are empty has been removed.
  // The calculation functions below are now expected to safely handle empty arrays.
...
export const calculateAcquisitionAnalyticsData = (tasks) => {
  if (!tasks || tasks.length === 0) {
    return {
      ...
      casinoBiaxialData: [
        { name: "No data available", tasks: 0, hours: 0, color: "#6b7280" },
      ],
      sportBiaxialData: [
        { name: "No data available", tasks: 0, hours: 0, color: "#6b7280" },
      ],
...
export const calculateProductAnalyticsData = (tasks) => {
  if (!tasks || tasks.length === 0) {
    return {
      ...
      productPieData: [
        { name: "No data available", value: 0, color: "#6b7280" },
      ],
      biaxialData: [
        { name: "No data available", tasks: 0, hours: 0, color: "#6b7280" },
      ],
...
export const calculateAIAnalyticsData = (tasks, users) => {
  if (!tasks || tasks.length === 0) {
    return {
      tableData: [
        {
          ...
          aiUsedTasks: 0,
          aiTime: 0,
          aiModels: "No data availableS",
          markets: "No data availableS",
          products: "No data availableS",
          aiUsagePercentage: 0,
          noData: true,
        },
      ],
      ...
      usersBiaxialData: [
        { name: "No data available", aiTime: 0, aiTasks: 0, color: "#6b7280" },
      ],
  ...
  if (!hasAIUsage) {
    return {
      ...
      usersBiaxialData: [
        { name: "No AI usage found", aiTime: 0, aiTasks: 0, color: "#6b7280" },
      ],
...
export const calculateReporterAnalyticsData = (tasks, reporters) => {
  ...
  const reporterTableColumns = [
    ...
    },
    {
      ...
      align: "left",
      render: (value, row) => {
        if (!value || value === "No data available" || value === "All Markets")
          return value;
        const markets = value.split(", ").filter((m) => m.trim());
        return (
          <div className="flex flex-wrap gap-1">
    ...
    },
    {
      ...
      align: "left",
      render: (value, row) => {
        if (!value || value === "No data available" || value === "All Products")
          return value;
        const products = value.split(", ").filter((p) => p.trim());
        return (
          <div className="flex flex-wrap gap-1">
