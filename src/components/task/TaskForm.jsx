...
const TaskForm = ({ onSubmit: customOnSubmit, initialValues: customInitialValues, isEdit = false }) => {
	...
	const validationSchema = Yup.object({
		jiraLink: Yup.string().url('Must be a valid URL').required('Jira link is required'),
		...
		timeInHours: Yup.number().required('Task completion time is required').min(0.1, 'Time must be at least 0.1 hours'),
	...
	const handleSubmit = async (values, { setSubmitting, resetForm }) => {
		try {
			...
			console.log('[TaskForm] submit', { isEdit, id: initialValues.id, monthId, taskData });
			if (customOnSubmit) {
				await customOnSubmit(taskData);
			} else if (isEdit && initialValues.id) {
				if (creatingRef.current) return;
				creatingRef.current = true;
				await dispatch(updateTask(monthId, initialValues.id, taskData));
			} else {
				if (creatingRef.current) return;
				creatingRef.current = true;
			...
			if (error?.code === 'month-not-generated' || error?.message === 'MONTH_NOT_GENERATED') {
	...
	const renderField = (field) => {
		...
		const baseInputClasses = `w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors ${hasError ? 'border-red-500' : 'border-gray-300'}`;
		return { baseInputClasses, hasError };
	};

	return (
		<div className="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-lg">
			<h2 className="text-2xl font-bold mb-6 text-gray-900">{isEdit ? 'Edit Task' : 'Create New Task'}</h2>
					...
					<Form className="space-y-6">
						<Field name="jiraLink">
							{(field) => {
								...
								return (
									<div>
										<label className="block text-sm font-medium text-gray-700 mb-1">Jira Link *</label>
										<input {...field.field} type="url" placeholder="https://your-domain.atlassian.net/browse/TASK-123" className={baseInputClasses} />
							...
							{(field) => (
								<div>
									<label className="flex items-center space-x-2">
										<input {...field.field} type="checkbox" checked={field.field.value} className="rounded border-gray-300 text-blue-600 focus:ring-blue-500" />
							...
							<>
								<Field name="timeSpentOnAI">
									{(field) => {
										...
										return (
											<div>
												<label className="block text-sm font-medium text-gray-700 mb-1">Time Spent on AI (hours) *</label>
												<input {...field.field} type="number" step="0.1" min="0" placeholder="e.g., 2.5" className={baseInputClasses} />
						...
						<Field name="timeInHours">
							{(field) => {
								...
								return (
									<div>
										<label className="block text-sm font-medium text-gray-700 mb-1">Task Completion Time (hours) *</label>
										<input {...field.field} type="number" step="0.1" min="0.1" placeholder="e.g., 8.0" className={baseInputClasses} />
