...
const MultiSelectField = ({
  ...
  useEffect(() => {
    if (watchedValue !== undefined && !Array.isArray(watchedValue)) {
      logger.warn(
        `MultiSelectField: Unexpected non-array value for ${field.name}:`,
        watchedValue,
        "Fixing to empty array"
      );
      setValue(field.name, [], { shouldValidate: false });
    }
  }, [watchedValue, field.name]);
