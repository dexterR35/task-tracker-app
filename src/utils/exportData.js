 ...
 */
const formatValueForCSV = (value, columnId, reporters = [], users = [], row = null) => {
  ...
  if (columnId === 'createdByName') {
    ...
    if (valueToCheck && users.length > 0) {
      ...
      const isLikelyUID = typeof valueToCheck === 'string' &&
                          (valueToCheck.length > 20 || valueToCheck.match(/^[a-zA-Z0-9]{20,}$/));

      // Try to find user by UID
      const userByUID = userUID ? users.find(u => {
      ...
      const userByName = !isLikelyUID && value ? users.find(u => {
  ...
  if (columnId === 'reporters') {
    ...
    if (typeof value === 'string' && !value.includes('@') && !value.includes('UID') && !value.match(/^[a-zA-Z0-9]{20,}$/i)) {
      return value;
    }
    ...
    try {
      ...
      const rows = data.map(row => {
        return allColumns.map(col => {
          ...
          const columnId = col.id || (col.accessorKey?.includes('.') ? col.accessorKey.split('.').pop() : col.accessorKey) || '';
          const formattedValue = formatValueForCSV(value, columnId, reporters, users, row);

          // Escape delimiter, quotes, and newlines in string values
          const delimiter = EXPORT_CONFIG.CSV_DELIMITER;
          if (formattedValue.includes(delimiter) || formattedValue.includes('"') || formattedValue.includes('\n')) {
      ...
      const blob = new Blob([csvContent], { type: `text/csv;charset=${EXPORT_CONFIG.CSV_ENCODING};` });
  ...
  try {
    ...
    if (Array.isArray(data)) {
      ...
      if (data.length === 0) {
        ...
        if (includeHeaders) {
          csvContent = [headers.join(EXPORT_CONFIG.CSV_DELIMITER), ...rows].join('\n');
        } else {
      ...
      const rows = entries.map(([key, value]) => {
        ...
        const escapedValue = stringValue.includes(delimiter) || stringValue.includes('"') || stringValue.includes('\n')
